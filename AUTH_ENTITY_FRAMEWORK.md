...
`UserRole` — C#-перечисление с допустимыми ролями. В базе роль хранится как текстовое значение, поэтому `HasConversion<string>()` говорит EF Core сохранять и читать enum как строку.
