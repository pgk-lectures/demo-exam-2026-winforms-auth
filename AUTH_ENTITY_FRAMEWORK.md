...
# 87. Если вы видите EF Core впервые
...
# 88. Почему User нужен, если пользователь уже есть в базе
...
# 89. Что такое DbContext простыми словами
...
# 92. Что означает user => user.Login
...
# 93. Почему User? может быть null
...
# 94. Почему гость передаётся как null
...
# 96. Почему сначала нужно посмотреть базу
...
# 97. Почему не нужно создавать пользователей в User.cs
...
# 98. Почему запрос находится в AuthService
...
# 99. Что происходит после нажатия «Войти»
...
# 100. Как искать ошибку, если авторизация не работает
...
# 101. Что делать, если роль не определяется
...
# 103. Что нужно понимать перед ДЭ
...
# 104. Почему EF Core рекомендуется именно здесь
...
# 105. Что ответить преподавателю
